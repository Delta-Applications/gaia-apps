...
(function(exports) {
  ...
  FocusManager.prototype.resetCurrentFocusedItems = function(fixed) {
    ...
    } else if (this.focus === StatusManager.STATUS_FAVORITE_SHOWING
        && this.arrow && this.previousFocusedIndex !== 0
        && this.previousFocusedIndex !== currentFocusList.elements.length - 1) {
        let frequency = FrequencyList.getFrequencyByElement(currentFocusList.elements[this.previousFocusedIndex]);
        mozFMRadio.enabled && mozFMRadio.setFrequency(frequency);
        return;
    }
  ...
  FocusManager.prototype.focusNext = function(next) {
    ...
    this.arrow = next ? 'arrowDown' : 'arrowUp';
