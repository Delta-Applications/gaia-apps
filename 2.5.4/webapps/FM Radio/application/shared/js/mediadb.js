...
var MediaDB = (function() {
  ...
  function MediaDB(mediaType, metadataParser, options) {
    ...
    openRequest.onerror = function(e) {
      dispatchEvent(media, 'openMediaDBFail');
  ...
  MediaDB.prototype = {
    ...
    // send out a mediadb event to the application UI.
    //
    appendFile: function appendFile(filename, file, callback, errcallback) {
      if (this.state !== MediaDB.READY) {
        throw Error('MediaDB is not ready. State: ' + this.state);
      }
      ...
      // Refetch the default storage area, since the user can change it
      // in the settings app.
      const storage = navigator.getDeviceStorage(this.mediaType);
      append();
      function append() {
      ...
      if (media.details.firstscan) {
        ...
        request.onerror = function(event) {
          ...
          if (request.error.name === 'ConstraintError') {
            ...
            var dbVersion =
              (0xFFFF & media.version) << 16 | (0xFFFF & MediaDB.VERSION);
            var reopenRequest = indexedDB.open(media.dbname, dbVersion);
            reopenRequest.onsuccess = function() {
              media.db = reopenRequest.result;
              next();
            };

            reopenRequest.onerror = function(e) {
              console.error('MediaDB reopenRequest error:',
                reopenRequest.error.name);
              next();
            };

            reopenRequest.onblocked = function(e) {
