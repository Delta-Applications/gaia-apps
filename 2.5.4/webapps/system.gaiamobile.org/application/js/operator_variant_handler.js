  ...
  OperatorVariantHandler.prototype = {
    ...
    applySettings: function ovh_applySettings(mcc, mnc, persistKeyNotSet) {
      ...
      this.retrieveOperatorVariantSettings(function(result) {
        /* FIH wangzhigang. modify for BTS-2284 begin */
        var config = null;
        var apnconfigname = 'apn_default_set_' 
                                       + this._iccCardIndex 
                                       + '_' + this._iccId;
        var self = this;
        var transaction = settings.createLock();
        var request = transaction.get(apnconfigname);
        request.onsuccess = function() {
            config = request.result[apnconfigname];
            if (config == 'true') {
                console.log('operator_variant_handler system, ' 
					+ 'retrieveOperatorVariantSettings, apn default config is true ');
                transaction.set({apnconfigname: 'false'});
                if (!persistKeyNotSet) {
                  self.retrieveWAPUserAgentProfileSettings(
                    self.applyWAPUAProfileUrl.bind(self)
                  );
                } else {
                  self.applyCellBroadcastSearchList(result);
                }
            } else {
               console.log('operator_variant_handler system, ' 
					+ 'retrieveOperatorVariantSettings, apn default config is not true');
                if (!persistKeyNotSet) {
                  self.applyOperatorVariantSettings(result);
                  self.retrieveWAPUserAgentProfileSettings(
                    self.applyWAPUAProfileUrl.bind(self)
                  );
                } else {
                  self.filterApnsByMvnoRules(0, result, [], '', '', [],
                    (function onFinishCb(filteredApnList) {
                      self.buildCompleteDefaultApnSettings(filteredApnList);
                      self.buildApnSettings(filteredApnList);
                  }).bind(self));
                  self.applyCellBroadcastSearchList(result);
                }
            }
        ...
        request.onerror = function() {
            console.log('operator_variant_handler system, ' 
                       + 'retrieveOperatorVariantSettings, request apn default config errot');
            if (!persistKeyNotSet) {
              self.applyOperatorVariantSettings(result);
              self.retrieveWAPUserAgentProfileSettings(
                self.applyWAPUAProfileUrl.bind(self)
        ...
        /* FIH wangzhigang. modify for BTS-2284 end */
      }.bind(this));
    ...
    filterApnsByMvnoRules: function ovh_filterApnsByMvnoRules(apnIndex,
      ...
      if (apnIndex === allApnList.length) {
        ...
        var transaction = settings.createLock();
        var request = transaction.get('operatorvariant.mvno');
        request.onsuccess = (() => {
          var mvnoInfos = request.result['operatorvariant.mvno'];
          if (!mvnoInfos || !Array.isArray(mvnoInfos)) {
            mvnoInfos = [null, null];
          }
          mvnoInfos[this._iccCardIndex] = mvnoMatchedInfo;
          var mvnoSettings = {};
          mvnoSettings['operatorvariant.mvno'] = mvnoInfos;
          transaction.set(mvnoSettings);
        });
      ...
      if (listMvnoType === 'gid' || listMvnoType === 'imsi') {
        listMvnoMatchData = listMvnoMatchData && listMvnoMatchData.toLowerCase();
      }
      ...
      request.onsuccess = (function onSuccessHandler() {
        ...
        if (!mvnoMatchedInfo.find((matchItem) => {
          return JSON.stringify(matchItem) === JSON.stringify(matchedInfo);
        })) {
    ...
    buildApnSettings: function ovh_buildApnSettings(allApnList) {
      ...
      navigator.hasFeature('device.capability.cdma_apn.feature').then((isEnabled) => {
        if (isEnabled) {
    ...
    resetDefaultApnSettings: function ovh_resetDefaultApnSettings(apnSettings) {
      let transaction = window.navigator.mozSettings.createLock();
      let request = transaction.get('ril.data.default.apnSettings');
      request.onsuccess = (() => {
        let result = request.result['ril.data.default.apnSettings'];
    ...
    buildPreferredApnSettings:
      ...
      var settings = window.navigator.mozSettings;
      var transaction = settings.createLock();
      var request = transaction.get('ril.data.dm.apnSettings');
      request.onsuccess = (() => {
        var result = request.result['ril.data.dm.apnSettings'];
        if (!result || !Array.isArray(result)) {
          result = [[], []];
        }
        // Store from apn.json if plmn change.
        result[this._iccCardIndex] = apnSettings;
        transaction.set({
          'ril.data.dm.apnSettings': result,
          'apn.selections': null
        });
    ...
    resetCpApns: function ovh_resetCpApns() {
      var settings = window.navigator.mozSettings;
      var transactionCpApns = settings.createLock();
      var requestCpApns = transactionCpApns.get('ril.data.cp.apns');
      requestCpApns.onsuccess = (() => {
        var result = requestCpApns.result['ril.data.cp.apns'];
        if (!result || !Array.isArray(result)) {
          result = [{}, {}];
        }
        result[this._iccCardIndex] = {};
        transactionCpApns.set({
          'ril.data.cp.apns': result
        });
      }).bind(this);
    },
    ...
    /**
     * Save the filtered and all data call settings to mozSettings.
     */
    saveDefaultApnSettings:
      function ovh_saveDefaultApnSettings(apnSettings, allApnSettings) {
        ...
        request.onsuccess = (() => {
          ...
          var allResult = [[], []];
          allResult[this._iccCardIndex] = allApnSettings;
          allResult[this._iccCardIndex^1] = result[this._iccCardIndex^1];

          result[this._iccCardIndex] = mergedApnSettings;
          transaction.set({
            'ril.data.dm.apnSettings': allResult,
            'apn.selections': null
          });
