(function (exports) {
  ...
  /**
   * Check if net_error is coming from a hosted app.
   */
  function isHostedApp(error) {
    var manifestURL = error.m;
  ...
  NetOfflineAppErrorView.prototype.addHandlers = function noew_addHandlers() {
    ...
    settingsBtn.onclick = showSettingsView;
    let failedLogoElement = document.getElementById('failed-logo');
    failedLogoElement.classList.add('offline');
    // Apply theme to be system.
    // Remove failed logo, provide refresh/quit buttons for hosted app.
    if (isHostedApp(this.error)) {
      failedLogoElement.style.background = 'none';
      failedLogoElement.classList.add('no-logo');
      let bodyElement = document.getElementById('net-error');
      bodyElement.classList.remove('theme-browser');
      bodyElement.classList.add('theme-system');
      let refreshBtn = document.getElementById('refresh-btn');
      refreshBtn.classList.remove('hidden');
      refreshBtn.onclick = function refreshBtnOnclick () {
        NetError.reload(true);
      };
      let quitBtn = document.getElementById('quit-btn');
      quitBtn.classList.remove('hidden');
      quitBtn.onclick = closeWindow;
    }
  };
  ...
  NetOfflineFramedErrorView.prototype.populateMessages =
    function noew_populateMessages() {
      localizeElement(this.title, 'network-error-launching-title');
      localizeElement(this.message, 'network-error-launching-message');
  };
  ...
  var ErrorViewFactory = {
    create: function nef_create() {
      ...
      return new view[getFrameType(error)](error, view.title, view.message);
    }
  };
  ...
  function initPage() {
    ...
    //we need to send information to application that net error was done, in default mozilla was shown dialogue with it feature phone don't show dialogue but lose focus in window
    //for example contacts application after merge Bugzilla --- Bug 1470
    window.parent && window.parent.postMessage('net-error', (window.location + ''));
    window.blur(); //iframe lose focus
  }
