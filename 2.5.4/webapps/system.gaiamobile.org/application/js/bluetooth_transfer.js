...
/* global CustomDialog, NfcHandoverManager, MimeMapper, uuid,
          MozActivity, NfcHandoverManager, Service, attentionWindowManager*/
...
var BluetoothTransfer = {
  ...
  _delayTime: 60000,
  _translate: navigator.mozL10n.get,
  _notificationTone: null,
  ...
  init: function bt_init() {
    ...
    SettingsListener.observe('notification.ringtone', null, (value) => {
  ...
  showReceivePrompt: function bt_showReceivePrompt(evt) {
    ...
    // Don't receive the file at the attention screens.
  ...
  playNotificationTone: function() {
    ...
    var settingsUrl = new SettingsURL();

    ringtonePlayer.src = settingsUrl.set(this._notificationTone);
  ...
  _onUpdateProgress: function bt__onUpdateProgress(mode, evt) {
    switch (mode) {
      case 'start':
        if (evt.detail.transferInfo.received ||
          this._sendingFilesQueue[0].numberOfFiles === 1 ||
          this._sendingFilesQueue[0].numSuccessful + this._sendingFilesQueue[0].numUnsuccessful < 1) {
            this._started = true;
            if (!this._isUserCanceled) {
              if (!evt.detail.transferInfo.received) {
                this.showStartNotification(evt.detail.transferInfo);
              }
  ...
  updateProgress: function bt_updateProgress(evt) {
    ...
    var info = {
      id: evt.address,
      title: evt.received ? this._translate('bluetooth-receiving-progress') : this._translate('bluetooth-sending-progress'),
      icon: 'style/bluetooth_transfer/images/icon_bluetooth.png',
...
//     miaoyj modify for LIO-1131:When the alarm goes off, the Bluetooth file transfer fails without any prompt start
//     // Don't show the transfer fail notice/toast in the attention screens and passcode lockscreen.
//     if (!transferInfo.success &&
//      (Service.query('locked') || attentionWindowManager.hasActiveWindow())) {
//      return;
//    }
//     miaoyj modify for LIO-1131:When the alarm goes off, the Bluetooth file transfer fails without any prompt end

     // Don't show the transfer fail notice/toast in the attention screens.
//     if (!transferInfo.success && attentionWindowManager.hasActiveWindow()) {
//      return;
//    }

    // If the file is very small, the gecko will not send the progress event, but need show toast
    if (this._started) {
      Service.request('SystemToaster:show', {
        text: this._translate('transfer-has-started-toast')
      });
    ...
    if (transferInfo.success) {
      ...
      if (transferInfo.received) {
        nData.title = this._translate('transferFinished-receivedSuccessful-title');
  ...
  showUnknownMediaPrompt: function bt_showUnknownMediaPrompt(fileName) {
    let body = this._translate('unknownMediaTypeToOpenFile',{fileName: fileName});
    Service.request('DialogService:show', {
