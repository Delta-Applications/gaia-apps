...
var FotaStub = {
  ...
  init: function fota_stub_init() {
    ...
    AlarmMessageHandler.removeCallback(this._preInitAlarmBindCb);
    if (this._isAlarmMissed) {
      // Alarm fired, need to schedule check
      this.debug('Alarm already fired, schedule check now');
      this._isAlarmMissed = false;
      this._nextAutoUpdateInfo.isAlarmAutoTriggered = true;
      this._fota.scheduleCheck();
      return;
    }
  ...
  onUpdateFound: function fota_on_update_found() {
    ...
    this._isAutoDownload = checkResult.autoDownload;
    // Check if an auto install package has been downloaded, need to clean it
    this.cleanAutoInstall();
  ...
  cleanAutoInstall: function fota_clean_auto_install() {
    if (this.autoInstall) {
  ...
  preInitAlarmCb: function fota_preInit_alarm_cb(alarm) {
    if (alarm.data.isFota2AutoUpdateAlarm) {
      this._isAlarmMissed = true;
    }
  },
...
function initFota() {
  dump("t2m_init initFota");
  if (!navigator.OmaService) {
    FotaStub.init();
    return;
  }

  const FOTA_VENDOR_KEY = 'deviceinfo.product_fota';

  function loadRedbendFotaHandler() {
    LazyLoader.load('js/fota_redbend_system.js', function() {
      redbendFotaUIEvent.init();
    });
    if (FotaStub._preInitAlarmBindCb) {
      AlarmMessageHandler.removeCallback(FotaStub._preInitAlarmBindCb);
    }
  }

  function fotaVendorObserver(e) {
    if (e.settingName === FOTA_VENDOR_KEY) {
      if ('redbend' === e.settingValue) {
        loadRedbendFotaHandler();
      }
      navigator.mozSettings.removeObserver(FOTA_VENDOR_KEY, fotaVendorObserver);
    }
  }

  let req = navigator.mozSettings.createLock().get(FOTA_VENDOR_KEY);
  req.onsuccess = ()=> {
    let fotaName = req.result[FOTA_VENDOR_KEY];
    if ('redbend' === fotaName) {
      loadRedbendFotaHandler();
      return;
    } else {
      // FOTA_VENDOR_KEY(deviceinfo.product_fota) may not set ready yet.
      navigator.mozSettings.addObserver(FOTA_VENDOR_KEY, fotaVendorObserver);
    }
    FotaStub.init();
  };
  req.onerror = ()=> {
...
window.addEventListener('ftudone', function initFota2Stub(evt) {
  ...
  let newcode =  navigator.engmodeExtension.getPrefValue("fota.commercial.ref", "na");
  dump(`t2m_init ftudone newcode: ` + newcode);
  if (newcode !== "na") {
    initFota();
  }
});
...
window.addEventListener('ftuskip', function initFota2Stub(evt) {
  ...
  //FotaStub.t2m_init();
  let newcode =  navigator.engmodeExtension.getPrefValue("fota.commercial.ref", "na");
  dump(`t2m_init ftuskip newcode: ` + newcode);
  if (newcode !== "na") {
    initFota();
  }
});

window.addEventListener("load", function fotaStubLoaded() {
  window.removeEventListener("load", fotaStubLoaded);
  dump(`t2m_init load` );
  FotaStub.t2m_init();
});

FotaStub._preInitAlarmBindCb = FotaStub.preInitAlarmCb.bind(FotaStub);
AlarmMessageHandler.addCallback(FotaStub._preInitAlarmBindCb);
